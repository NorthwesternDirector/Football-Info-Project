...
const Welcome = props => {
  ...
  <PageHeaderWrapper title="ND学习时间" subTitle="我又不是个薯憨憨，当然会选择做前端">
    <Row gutter={24}>
      <Col span={6}>
        <ReactEcharts theme="theme" option={option} style={{ height: '300px' }} ref={chartsRef}></ReactEcharts>
      </Col>
      <Col span={8}>
        <Card
        bodyStyle={{ padding: 0 }}
          ...
          <Carousel autoplay style={{ height: 220 }}>
            <div>
              {iconInfo && iconInfo.data ? iconInfo.data.map(item =>
                <>
                  <Avatar size="large" style={{ backgroundColor: '#FFF' }} icon={<IconFont type={item.type} />} />
                  <Tag>{item.info}</Tag>
                </>) : null
              }
            </div>
